...
namespace xpdm.Bitcoin.Messaging
{
    ...
    {
        public static T[] ReadArray<T>(this byte[] buffer, int offset, ulong count) where T : SerializableMessageBase
        {
            ...
            {
                retVal[i] = SerializableMessageBase.ReadFromBitcoinBuffer<T>(buffer, offset);
                offset += (int)retVal[i].ByteSize;
            }
            return retVal;
        }

        public static void WriteBytes<T>(T[] val, byte[] buffer, int offset, ulong count) where T : SerializableMessageBase
        {
