...
namespace xpdm.Bitcoin.Protocol
{
    public class TxIn : BitcoinSerializableBase
    {
        ...
        public TxIn(TxOutpoint outpoint, byte[] script, uint sequence)
        {
            ...
            ByteSize = Outpoint.ByteSize + ScriptLength.ByteSize + (uint)ScriptLength.Value * BitcoinBufferOperations.UINT8_SIZE + Sequence.ByteSize();
        }
            ...
            :base(buffer, offset)
        {
            ...
            _script = script;
            Sequence = buffer.ReadUInt32(offset + SCRIPTLEN_OFFSET + (int)ScriptLength.ByteSize + (int)ScriptLength.Value * BitcoinBufferOperations.UINT8_SIZE);
        ...
        public override void WriteToBitcoinBuffer(byte[] buffer, int offset)
        {
            ...
            Sequence.WriteBytes(buffer, offset + SCRIPTLEN_OFFSET + (int)ScriptLength.ByteSize + (int)ScriptLength.Value * BitcoinBufferOperations.UINT8_SIZE);
        }
        ...
        public static int MinimumByteSize
        {
            get
            {
                return TxOutpoint.ConstantByteSize + VarInt.MinimumByteSize + BitcoinBufferOperations.UINT32_SIZE;
            }
        }
